...
class Login extends CI_Controller {
    ...
    public function validate_login($from = "") {
        ...
        $credential = array('mobile_no' => $mobile_no, 'password' => sha1($password), 'status' => 1);
    ...
    public function validate_login1($from = "") {
...
// print_r($data);die;
    ...
    public function sentotp() {
        ...
        $validity = $this->user_model->check_duplication_mobile('on_create', $data['mobile_no']);
        if ($validity) {
            // $emailvalidity = $this->user_model->check_duplication('on_create', $data['email']);
                ...
                $this->session->set_userdata('pincode',$data['pincode']);
                $this->session->set_userdata('verification_code',$verification_code);

                $msg     =  "Hello user, Your verification OTP ".$verification_code;
                
                $this->send_otp($data['mobile_no'], $msg);

                $this->session->set_flashdata('flash_message', "Your otp has send to your mobile number. ". $verification_code.'.');
                $array['status'] = 'success';
                $array['message'] = 'Your otp has send to your mobile number.';
                $array['verification_code'] = $verification_code;
        }else {
            $this->session->set_flashdata('error_message', 'Mobile number already exist');
              $array['status'] = 'error';
              $array['message'] = 'Mobile number already exist';
        }
    ...
    public function otpVerify(){
        $array = [];
        $data['mobile_otp'] = html_escape($this->input->post('otp'));
        $arr = $this->session->userdata();
        // print_r($arr);die;
        $data['last_name']=    html_escape($arr['last_name']);
        $data['first_name']=   html_escape($arr['first_name']);
        $data['email']=        html_escape($arr['email']);
        $data['password']=     html_escape($arr['password']);
        $data['mobile_no']=    html_escape($arr['mobile_no']);
        $data['state']=        html_escape($arr['state']);
        $data['district']=     html_escape($arr['district']);
        $data['pincode']=      html_escape($arr['pincode']);
        $data['verification_code']= html_escape($arr['verification_code']);

        if ($data['mobile_otp'] != $data['verification_code'] ) {
            $this->session->set_flashdata('error_message', get_phrase('wrong_otp'));
                  $array['status'] = "error";
            ...
            $array['status'] = "success";
            $array['message'] =  get_phrase('your_registration_has_been_successfully_done');
        }
        echo json_encode($array);
    }
    ...
    public function send_otp($mobile_no = "9131970206",$msg="Hello user, Your verificarion OTP"){
       
        $auth_key = "d61ad9ea5fa5776ad9f2e4161a43874";
        
        $message = $msg;

        $sender = "TBTSIG"; //ex:INVITE

        $mobile_number = $mobile_no;
      
        $url = "http://sms.fbipool.com/rest/services/sendSMS/sendGroupSms?AUTH_KEY=".$auth_key."&message=".urlencode($message)."&senderId=".$sender."&routeId=1&mobileNos=".$mobile_number."&smsContentType=english";
